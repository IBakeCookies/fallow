...
| `stop-block-structure.probe.ts`           | §8.10 — the day's own breaks against the summed reading, per λ₀: the error distribution, order vs breaks, log jitter, the batch fallback, the oracle                                            |
...
| `enb-break-economics.probe.ts`            | §8 intro, §8.3–8.4, §13.5 — break economics pre/post fix, fragmentation cost, chunk sweep, and what the SHIPPED default λ₀ plans by portfolio and window                                        |
...
| `stp-stopping-identifiability.probe.ts`   | §8.10 — V_T identifiability on one day and over 300 seeded ones, and the reconstruction's bracket                                                                                               |
