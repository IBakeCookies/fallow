 ...
 *
 * MATH.md §19.4 is the sole surviving justification for the cap. It states a
 ...
 *      slider (0.25–24h, 0.25h steps) — plus the n = 5 rung §19.4 skips, and
 ...
 *      that reproduces the ladder §19.4 first quoted
 *   2  the same sweep at the σ_ϕ a fit actually produces. §19.4 says "fitted"
 ...
 *  active on every task — "ϕ̂ held at the 0.1h floor" in §19.4's sense. */
...
describe('GAIN_PERCENT_CAP: what reaches it, and can a fit get there?', () => {
	...
	it('arm 2 — the same ladder at the σ_ϕ the fit actually produces', () => {
		...
		// §19.4's own words as a history: every log 15–30m, spread over the
	...
	it('arm 3 — at default constants the cap is out of reach', () => {
		...
		// §19.4 quoted 569% here. The readings that could have produced it, so the
...
/**
 * A synthetic FAST-flow user — §19.4's "logging 15–30m everywhere".
...
function recordFit(tally: FloorTally, logs: FlowObservation[], arm: string): void {
	...
	// A fit that fell back to the defaults is not a fitted user, and §19.4's
