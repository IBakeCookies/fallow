...
import Harness from './theme-store.test-harness.svelte';
...
function mount(props: { initialTheme?: ThemeName; initialSceneryPaused?: boolean }): ThemeStore {
	...
	render(Harness, { ...props, onstore: (created: ThemeStore) => (store = created) });
	return store;
}
...
describe('ThemeStore appearance reconciliation', () => {
	...
	it('lets the cookie override a stale SSR theme', async () => {
		storeCookie({ theme: 'abyss' });

		expect(mount({ initialTheme: 'parchment' }).theme).toBe('abyss');
	});

	it('lets the cookie override a stale SSR scenery-motion setting', async () => {
		storeCookie({ sceneryPaused: false });
		expect(mount({ initialSceneryPaused: true }).sceneryPaused).toBe(false);

		storeCookie({ sceneryPaused: true });
		expect(mount({ initialSceneryPaused: false }).sceneryPaused).toBe(true);
	});

	it('keeps the SSR value when no cookie records a preference', async () => {
		expect(mount({ initialSceneryPaused: true }).sceneryPaused).toBe(true);
	});
});
