<script module lang="ts">
	...
	import EnergyChart from './energy-chart.svelte';
	...
	const { Story } = defineMeta({
		...
		tags: ['autodocs'],
		args: { trajectory: buildTrajectory(12), windowHours: 12 }
	});
</script>
...
<Story name="Sixteen hour window" args={{ trajectory: buildTrajectory(16), windowHours: 16 }}>
	{#snippet template(args)}
		<div class="max-w-3xl rounded-2xl border bg-surface-card p-box-xl backdrop-blur shadow-card">
			<EnergyChart {...args} />
		</div>
...
<Story name="Nothing planned" args={{ trajectory: [], windowHours: 8 }}>
	{#snippet template(args)}
		<div class="max-w-3xl rounded-2xl border bg-surface-card p-box-xl backdrop-blur shadow-card">
			<EnergyChart {...args} />
		</div>
