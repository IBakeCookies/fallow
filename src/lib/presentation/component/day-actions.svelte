<script lang="ts">
	...
	// The seam of the Save|Load pair: ends round at the sm button's own capped radius,
	// neighbours share one border (`-ms-px`), and a focused segment rides over it.
	const joined =
		'relative -ms-px rounded-none first:ms-0 first:rounded-s-[min(var(--radius-md),10px)] last:rounded-e-[min(var(--radius-md),10px)] focus-visible:z-10';
...
<div class={cn('flex flex-wrap items-center justify-start sm:justify-end gap-grid-xs', className)}>
	...
	<!-- One pair, joined: a day is loaded and saved from the same two-button control, so
	     they share a seam rather than a gap — the importance segments' recipe (STYLE.md).
	     `{#if canSave}` can leave Load alone in it, which is why the ends round with
	     `first:`/`last:` and not per-button. -->
	<div class="flex">
		<DropdownMenu.Root bind:open={showLoadMenu}>
			<DropdownMenu.Trigger class={joined}>
