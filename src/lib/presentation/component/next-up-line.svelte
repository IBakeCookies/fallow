<script lang="ts">
	...
	/* "You are here": position 1 of the mid-day re-plan's run order (MATH.md §35).
	   It rides the list card's header row, not the rows themselves, because the
	   list's own `#N` badges are the whole-day plan's order and stay the 8am answer
	   as the day is worked (§11.8). It never says "stop for the day" — the classic
	   objective prices no leisure (§14.1), so day-ending belongs to λ₀ and the
	   Energy Lab's stop advisor (§8.11). */
