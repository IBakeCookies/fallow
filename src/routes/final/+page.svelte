<script lang="ts">
	...
	type TaskType = 'Cognitive' | 'Physical' | 'Hybrid';
	...
	const taskTypes: TaskType[] = ['Cognitive', 'Physical', 'Hybrid'] as const;
	...
	const burnoutRisk = $derived.by(() => {
		...
		// When E/β > 1, the task drains more than it rewards
		const totalNetStrain = activeTasks.reduce((sum, t) => {
			const strainFactor = t.trueEffort / t.trueEnjoyability; // E/β = 1/p₀
			const netStrain = Math.max(0, strainFactor - 1); // Only count drain above baseline
							...
							>
								<div class="flex items-center gap-4">
									...
									<div class="flex items-center gap-2">
										<span
											class="text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded {task.taskType ===
											'Cognitive'
												? 'bg-blue-500/20 text-blue-400'
												: task.taskType === 'Physical'
													? 'bg-emerald-500/20 text-emerald-400'
													: 'bg-violet-500/20 text-violet-400'}"
											title={task.taskType || 'Cognitive'}
										>
											{task.taskType === 'Cognitive'
												? 'COG'
												: task.taskType === 'Physical'
													? 'PHY'
													: 'HYB'}
								...
								<div class="flex items-center gap-2">
									...
									<span
										class="rounded border border-zinc-800 bg-zinc-900/50 px-2 py-0.5 text-xs font-medium text-zinc-400"
										>Priority: {task.priorityScore}</span
									>
									<span
										class="rounded border border-zinc-800 bg-zinc-900/50 px-2 py-0.5 text-xs font-medium text-zinc-400"
					...
					<div class="rounded-xl border border-white/10 bg-white/3 p-5 backdrop-blur-xl">
						<div class="flex items-center justify-between mb-4 border-b border-white/5 pb-4">
							<h4
								...
								title="Average enjoyment minus difficulty across all tasks. Upward = sustainable pace, Reset Reqd = potential burnout risk, Stable = balanced workload."
							>
								Momentum Vector
							</h4>
							<span
