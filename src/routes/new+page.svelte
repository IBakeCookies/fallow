<script lang="ts">
	...
	const burnoutRisk = $derived.by(() => {
		...
		console.log(highStrainHours);

		// Risk = % of day spent on high-strain tasks, scaled so 50% of day = 100% risk
		return Math.min(100, Math.round((highStrainHours / (budget * 0.5)) * 100));
	});
	...
	const metrics = $derived([
		...
		},
		{
			...
			value: `${burnoutRisk}%`,
			description:
				'Percentage of time spent on high-strain tasks (difficulty ≥7). Risk increases when this exceeds 50% of your day.',
						...
						{#each suggestedTasks as task (task.id)}
							...
							>
								...
								>
									...
									<span
										class="rounded border border-zinc-800 bg-zinc-900/50 px-2 py-0.5 text-xs font-medium text-zinc-400"
										>{decimalHourToMinutes(task.suggestedHours)}
									</span>
									<button
